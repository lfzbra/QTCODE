...
void VideoWindow::mediaPlayerInit()
{
...
void VideoWindow::btn_play_clicked()
{
...
void VideoWindow::btn_next_clicked()
{
    videoPlayer->stop();
...
void VideoWindow::btn_volmeup_clicked()
{
    /* 点击每次音量+5 */
...
void VideoWindow::btn_fullscreen_clicked()
{
    /* 全屏/非全屏操作 */
    vWidget[0]->setVisible(!pushButton[4]->isChecked());
}

void VideoWindow::btn_volmedown_clicked()
{
    /* 点击每次音量-5 */
    volumeSlider->setValue(volumeSlider->value() - 5);
    videoPlayer->setVolume(volumeSlider->value());
...
void VideoWindow::mediaPlayerStateChanged(
        QMediaPlayer::State
        state)
{
    switch (state) {
...
void VideoWindow::listWidgetCliked(QListWidgetItem *item)
{
    videoPlayer->stop();
...
void VideoWindow::mediaPlaylistCurrentIndexChanged(
        int index)
{
    if (-1 == index)
        return;
...
void VideoWindow::musicPlayerDurationChanged(
        qint64 duration)
{
...
void VideoWindow::mediaPlayerPositionChanged(
        qint64 position)
{
...
void VideoWindow::durationSliderReleased()
{
    /* 设置媒体播放的位置 */
    videoPlayer->setPosition(durationSlider->value() * 1000);
}

void VideoWindow::volumeSliderReleased()
{
    /* 设置音量 */
    videoPlayer->setVolume(volumeSlider->value());
}
