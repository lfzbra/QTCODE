...
//        //QString temp = getJsonValue(data, key1);
        QString temp;
        QJsonObject obj = QJsonDocument::fromJson(data).object();
        QJsonValue val = obj.value("result");
        QJsonValue first = val.toArray().at(0);
        temp=first.toObject().value("name").toString();
//        if(flag)
//            temp = "（动物）"+temp;
//        else
//            temp= "（植物）"+temp;
        ...
        qDebug()<< "识别结果为：" << temp<<endl;
       // tutitle->setText(temp);
    }
}
...
/* 开始识别 */
void Ocr::getTheResult(QString fileName,bool flag)
{
    QFile file;
    ...
    if(flag)
        ...
        serverApiUrl = QString(server_api1).arg(accessToken);

    requestNetwork(serverApiUrl, body);
}

/* 开始识别 */
void Ocr::getTheResult(QImage image,bool flag)
{
    QByteArray requestData;
...
/* 准备识别 */
void Ocr::readyToDetection(QString imagePath,bool flag)
{
    if (accessToken.isEmpty()) {
        qDebug() << "未获取到token!已返回！" <<endl;
        return;
    }
    getTheResult(imagePath,flag);
}

/* 准备识别 */
void Ocr::readyToDetection(QImage image,bool flag)
{
    if (accessToken.isEmpty()) {
        qDebug() << "未获取到token!已返回！" <<endl;
        return;
    }
    getTheResult(image,flag);
}
